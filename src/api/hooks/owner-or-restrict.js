...
module.exports = function ownerOrRestrict(options) {
  return function setup(hook) {
    if (hook.type !== 'before') {
      throw new Error(
        'The \'ownerOrRestrict\' hook should only be used as a \'before\' hook.'
      );
    }

    if (!hook.id) {
      throw new errors.MethodNotAllowed(
        'The \'ownerOrRestrict\' hook should only be used on the \'get\', \'update\', \'patch\' and \'remove\' service methods.'
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      return this.get(hook.id, params)
        .then((data) => {
          if (data.toJSON) {
            data = data.toJSON();
          ...
          if (Array.isArray(field)) {
            ...
            if (
              fieldArray.length === 0 || fieldArray.indexOf(id.toString()) < 0
            ) {
              const name =
              ...
              let restrictions = _.chain(options.restrictOn)
                .map(
                  restriction =>
                    (_.has(hook.data, restriction) ? restriction : null)
                )
                .compact()
                .value()
                .join(', ');
              ...
              throw new errors.Forbidden(
                `${name} is not permitted to update the ${restrictions} field${includeS}.`
              );
            }
          } else if (
            field === undefined || field.toString() !== id.toString()
          ) {
            const name =
              _.get(hook, 'params.user.name') ||
              _.get(hook, 'params.user.email');
